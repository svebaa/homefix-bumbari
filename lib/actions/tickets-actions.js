...
export async function getTicket(ticketId) {
  ...
  const { data: ticket, error: ticketError } = await supabase
    .from("ticket")
    .select(
      "ticket_id, title, description, issue_category, status, created_by, assigned_to, unit_id, created_at, resolved_at,comment"
    )
...
export async function assignContractor(ticketId, assigned_to) {
  ...
  const { data: contractor, error: contractorError } = await supabase
    ...
    .single();
  if (contractorError || !contractor) return { error: "Majstor nije pronađen." };

  // 6a) GENERAL majstor može na sve kategorije – preskačemo provjeru
  if (contractor.specialization !== "GENERAL") {
    ...
    const expectedCategory = specializationToCategory[contractor.specialization];

    // ako nema mapiranja ili ne odgovara kategoriji ticketa -> error
    if (!expectedCategory || expectedCategory !== ticket.issue_category) {
      return {
        error: `Nekompatibilno: kategorija kvara (${ticket.issue_category}) ≠ specijalizacija (${contractor.specialization}).`,
      };
    }
  }
...
// GET TICKETS FOR CONTRACTOR
export async function getTicketsForContractor() {
  const supabase = await createClient();
  ...
  if (userError || !user) {
    return { error: "Niste prijavljeni.", data: null };
  }
  ...
  if (profileError || !profile) {
    return { error: "Profil nije pronađen.", data: null };
  }

  if (profile.role !== "CONTRACTOR") {
    return { error: "Nemate ovlasti.", data: null };
  }

  // 3) ČITANJE IZ BAZE (KLJUČNI DIO)
  const { data, error } = await supabase
    .from("ticket")
    .select(
      "ticket_id, title, issue_category, status, created_at"
    )
...
// GET TICKET PHOTOS (for details page)
export async function getTicketPhotos(ticketId) {
  const supabase = await createClient();

  // Koristimo postojeću logiku ovlasti iz getTicket()
  const { error: ticketErr } = await getTicket(ticketId);
  if (ticketErr) return { error: ticketErr, data: null };
...
// UPDATE TICKET COMMENT (contractor note)
export async function updateTicketComment(ticketId, comment) {
  const supabase = await createClient();
  ...
  if (!isAssigned && !isAdmin) {
    return { error: "Nemate ovlasti za dodavanje napomene na ovaj ticket." };
  }

  const safeComment = (comment ?? "").toString();
