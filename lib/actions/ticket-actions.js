...
export async function getActiveTickets() {
    ...
    if (tickets && tickets.length > 0) {
        ...
        const contractorIds = tickets.map((t) => t.assigned_to).filter(id => id !== null);

        const [buildingUnitsResult, contractorsResult] = await Promise.all([
            supabase
                .from("building_unit")
                .select("unit_id, label, floor, building_id")
                .in("unit_id", buildingUnitIds),
            contractorIds.length > 0 
                ? supabase.from("profile").select("user_id, first_name, last_name").in("user_id", contractorIds)
        ...
        if (contractorsResult.data) {
            contractorsResult.data.forEach((contractor) => {
                contractorsMap[contractor.user_id] = `${contractor.first_name} ${contractor.last_name}`;
            });
        }
        ...
        tickets.forEach((ticket) => {
            ...
            ticket.contractor_name = contractorsMap[ticket.assigned_to] || null;
        });
    }
...
export async function getResolvedTickets() {
    ...
    if (tickets && tickets.length > 0) {
        ...
        const [buildingUnitsResult, ratingsResult, contractorsResult] = await Promise.all([
            supabase
                .from("building_unit")
                .select("unit_id, label, floor, building_id")
                .in("unit_id", buildingUnitIds),
